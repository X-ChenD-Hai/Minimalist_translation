...
public:
    ...
    bool mousePressDeal(qintptr *result);                         // 鼠标在按钮内按下处理
    bool mouseRealseDeal(qintptr *result, bool isNClient = true); // 鼠标释放处理
...
private:
    ...
    /**********在原来基础上添加***************/
    BtnHelper *mMinBtnHelper = nullptr;   // 最小化按钮
    BtnHelper *mMaxBtnHelper = nullptr;   // 最大化按钮
    BtnHelper *mCloseBtnHelper = nullptr; // 关闭按钮
};
